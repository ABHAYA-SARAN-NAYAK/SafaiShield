...
export function SessionProvider({ children }) {
  ...
  const startCheck = useCallback(() => {
    setSession(prev => ({
      ...DEFAULT_SESSION,
      localId: crypto.randomUUID?.() || Date.now().toString(36),
      active: true,
  ...
  const startDescent = useCallback(() => {
    ...
    // Send job_start Telegram alert
    sendTelegramAlert('job_start', {
      locationText: session.latitude && session.longitude ? `${session.latitude.toFixed(4)}, ${session.longitude.toFixed(4)}` : 'Unknown',
      riskTier: session.riskLevel || 'medium',
    }).catch(err => console.log('Telegram start alert error:', err))
  }, [session])
  ...
  const endSession = useCallback(() => {
    ...
    // Send job_safe_exit Telegram alert
    sendTelegramAlert('job_safe_exit', {
      locationText: session.latitude && session.longitude ? `${session.latitude.toFixed(4)}, ${session.longitude.toFixed(4)}` : 'Unknown',
    }).catch(err => console.log('Telegram exit alert error:', err))
